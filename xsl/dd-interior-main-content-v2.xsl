   ...
   <!-- alt for main img/media? can't require due to dd structure -->
    ...
    <xsl:template match="section-content" mode="standard">
        ...
        <xsl:variable name="element-name">
            <xsl:choose>
                <xsl:when test="section-heading/heading != ''">section</xsl:when>
        ...
        <xsl:element name="{$element-name}">
            ...
            <xsl:if test="$current-heading-level = 'Heading 2'"><xsl:attribute name="id">section-<xsl:value-of select="position()"/></xsl:attribute></xsl:if>
            ...
            <xsl:if test="section-heading/heading != ''">
                ...
                <xsl:element name="{$heading-element}"><xsl:value-of select="section-heading/heading"/></xsl:element>
            </xsl:if>
    
            <xsl:call-template name="section-inner"/>
            <xsl:apply-templates select="section-content-boxes"/>
            <xsl:apply-templates mode="standard" select="following-sibling::section-content[(section-heading/section-heading-level = $sub-heading-level or section-heading/heading = '') and preceding-sibling::section-content[section-heading/section-heading-level = $current-heading-level][1] = $current]"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template name="section-content-accordion">
        ...
        <xsl:if test="section-heading/heading!=''">
            <h2 class="accordion-heading">
                <a class="accordion-toggle" data-parent="#accordion" data-toggle="collapse" href="#collapse{position()}">
                    <span aria-hidden="true" class="fa fa-angle-right"></span><xsl:value-of select="section-heading/heading"/>
                </a>
            </h2>
        </xsl:if>
        ...
        <div id="collapse{position()}">
            ...
            <xsl:attribute name="class"> accordion-body collapse <xsl:if test="expanded/value = 'Yes'">in</xsl:if></xsl:attribute>
