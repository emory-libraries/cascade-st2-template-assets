    ...
    <xsl:template match="system-index-block">
        ...
        <xsl:variable name="lastYear" select="$currentDateTime - (31536000*1000)"/>
          <aside class="feed-icon pull-right">
            <p><a href="http://staging.web.emory.edu/librariestemplate/news1/index.rss"><span class="fa fa-rss-square"></span></a></p>
        </aside> 
        <dl class="dl-horizontal news-index">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:apply-templates select="descendant::system-page">
                        <xsl:sort order="descending" select="start-date"/>
                    </xsl:apply-templates>
                </xsl:otherwise>
            </xsl:choose>
