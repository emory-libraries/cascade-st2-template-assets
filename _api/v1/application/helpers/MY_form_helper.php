...
if ( ! function_exists('form_select'))
{
     ...
     * @see http://codeigniter.com/forums/viewthread/193529/
     */
    function form_select($name = '', $options = array(), $selected = array(), $extra = '')
    {
        if ( ! is_array($selected))
        {
            $selected = array($selected);
        ...
        $form = '<div class="check-box-field">'."\n";
        ...
        foreach ($options as $key => $val)
        {
            ...
            {
                $sel   = in_array($key, $selected) ? ' selected="selected"' : '';
                $form .= '<label><input name="'.$key.'" type="checkbox" value="' . $val . '"' . $sel . ' ' . $attrs . '/><span class="optionLabel">' . (string) $val . "</span></label>\n";
            }
        }
