...
class Form {
    ...
    {icon}
    <span class="text-error">{error_text}</span>
    ...
    /**
     * Stores subject fields template prototype.
     * @var string
     */
    private static $_subject_template = '
    {before}
    <{wrapper_element} id="{wrapper_id}" class="control-group {type}-control-wrapper{wrapper_class}{error}">
    {label}
    <div class="controls text-controls">
    <div class="text-input input-prepend{input_append}">
    {icon}
    {prefix}
    <input id="{id}" class="{class}" placeholder="{placeholder}" name="name__subject" type="text" value="{value}" {attributes} {disabled}{readonly}{checked}{selected}  />
    {suffix_inline}
    {help_inline}
    {help_block}
    </div> <!-- /.input-prepend -->
    ...
    /**
     * Stores a multiple select template prototype.
     * @var string
     */
    private static $_select_template = '
    {before}
    <{wrapper_element} id="{wrapper_id}" class="control-group {type}-control-wrapper{wrapper_class}{error}">
    <div class="controls {type}-controls">
    <div class="validate {type}-input input-prepend{input_append}">
    {label}
    {icon}
    {prefix}
    {options}
    ...
    <{element} id="{id}" type="submit" class="form-submit btn {class}" {attributes}>{value}</{element}>
    {suffix_inline}
    ...
    {icon}
    {prefix}
    <textarea id="{id}" class="{class}" placeholder="{placeholder}" name="{name}" type="{type}" data-validate="{rules}" {attributes} {disabled}{readonly}>{value}</textarea>
    {suffix_inline}
    {help_inline}
    {help_block}
    </div> <!-- /.input-prepend -->
    ...
    /**
     * Stores a bootstrap time template prototype.
     * @var string
     */
    private static $_datetime_template = '
    {before}
    <{wrapper_element} id="{wrapper_id}" class="control-group {type}-control-wrapper{wrapper_class}{error}">
    {label}
    <div class="controls {type}-controls">
    ...
    <a class="btn btn-small addField">Add another date/time</a>
    {suffix}
    </div> <!-- /.controls -->
    </{wrapper_element}> <!-- /.control-group -->
    {after}
    ...
    <input type="{toggle}" id="{id}-{index}{rand}" name="{name}" value="{value}" {attributes} {checked} data-validate="{rules}" /> {label}
    </label>
    {after}
    {private}
    ';
    ...
    public function __construct()
    {
        ...
        self::$CI->load->helper(array('form_helper', 'url_helper'));
    }
     ...
     * @return string
     */
    public static function get($form, $form_name, $render = TRUE, $flush = FALSE)
    {
        static $cache = array();
        ...
        {
            $cache[$form_name] = self::load($form, $form_name);
        }
     ...
     */
    public static function load($form, $form_name, $values = FALSE)
    {
     ...
     * @return boolean
     */
    public static function validate($form, $form_name, $CI = FALSE, $force_rules = array())
    {
    ...
    private static function _render_field($field, $name = FALSE, $template = FALSE)
    {
        ...
        /* TODO: note that required breaks on datetime fields. Form will not pass validation even if they are filled in. */

        // Build field label
        if (isset($field['label']) AND $field['label'] AND $field['type'] != 'button' AND ! isset($field['toggle']))
        {
            $field['label'] = '<label class="control-label" for="' . $field['id'] . '"><span>' . $field['label'] . $field['required'] . '</span><span class="text-error">'.$field['error_text'].'</span></label>';
        }
        ...
        isset($field['rules']) AND $field['rules'] = self::_prep_clientside_rules(substr($field['rules'], 0, -1));
    ...
    /**
     * Renders a subject element.
     *
     * @return string
     */
    private static function _render_subject($field, $name = FALSE, $multistep = FALSE)
    {
        return self::_render_field($field, $name, self::$_subject_template);
    }
    ...
    /**
     * Renders a Bootstrap DateTime element.
     *
     * @return string
     */
    private static function _render_datetime($field, $name = FALSE, $multistep = FALSE)
    {
        return self::_render_field($field, $name, self::$_datetime_template);
    }

    /**
     * Renders a Bootstrap date element.
     *
     * @return string
     */
    private static function _render_date($field, $name = FALSE, $multistep = FALSE)
    {
        return self::_render_field($field, $name, self::$_date_template);
    }
     ...
     * @return string
     */
    private static function _render_select($select, $name, $multistep = FALSE)
    {
        // Load dropdown data model if set.
        isset($select['data']) AND $select['options'] = self::_get_data($select['data']);

        // Prepare value
        isset($select['value']) OR $select['value'] = '';
        $select['value'] = self::value($name, $select['value']);

        // Handle dropdown attributes
        isset($select['attributes']) OR $select['attributes'] = array();
        $select['attributes'] += array('id' => 'edit-' . self::clean($name));

        // @TODO: Replace with data-attributes
        // Helps overcome dependent dropdowns issue on prepopulated options
        if (isset($select['cached_value']) AND $select['cached_value'])
        {
            $select['attributes'] += array(
                'cached_value' => is_array($select['value'])
                ? implode('|', $select['value'])
                : $select['value']
                );
        }

        // Parse attributes into string
        $attributes = _parse_form_attributes($select['attributes'], array());

        // Handle multiselect dropdowns
        $helper = (isset($select['multiselect']) AND $select['multiselect']) ? 'form_multiselect' : 'form_select';

        // Render select select and option elements
        isset($select['options']) AND
        $select['options'] = $helper($name, $select['options'], $select['value'], $attributes);

        // Inline element?
        $template = isset($select['inline'])
        ? self::$_dropdown_inline_template
        ...
        // not try to guess its possible values
        unset($select['value']);
        $select['no_value'] = TRUE;

        return self::_render_field($select, $name, $template);
    }
    // --------------------------------------------------------------------
    ...
    private static function _render_form_open($form, $multistep = FALSE, $manual = FALSE)
    {
        ...
        if ($form['ci_action'] === true) {
            ...
            $url = self::$CI->config->item('base_url');
            $action = 'action="'.str_replace($url, '', $form['action']).'"';
            $attributes = '';
            foreach ($form['attributes'] as $key => $value) {
                $attributes .= ' '.$key.'="'.$value.'"';
            }
            $multipart = '';
            if ($form['multipart'] === true) {
                $multipart = ' enctype="multipart/form-data"';
            }
            $output = '<form '.$action.$attributes.$multipart.' accept-charset="utf-8" method="post">';
        }
